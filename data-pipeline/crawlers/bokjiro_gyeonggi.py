...
SOURCE_NAME = "복지로_경기청년복지"
DATA_SCOPE = "경기"

OUTPUT_ACTIVE_PATH = "output/raw/bokjiro_gyeonggi_active_raw.csv"
OUTPUT_NEED_CHECK_PATH = "output/raw/bokjiro_gyeonggi_need_check_raw.csv"
OUTPUT_CLOSED_PATH = "output/raw/bokjiro_gyeonggi_closed_raw.csv"
OUTPUT_OLLAMA_INPUT_PATH = "output/raw/bokjiro_gyeonggi_raw_all.csv"

DEBUG_DIR = "output/debug"
COOKIE_PATH = "output/debug/bokjiro_gyeonggi_cookies.txt"
...
class BokjiroClient:
    ...
    def init_session(self):
        ...
        try:
            ...
            print("[복지로-경기] 세션 초기화 requests 실패, curl 재시도:", error)
        ...
        try:
            ...
            print("[복지로-경기] 세션 초기화 curl 실패, API 직접 호출로 진행:", error)
            return ""
    ...
    def fetch_text(self, method: str, url: str, headers=None, params=None, data=None, json_data=None):
        try:
            ...
            print("[복지로-경기] requests 실패, curl 재시도:", requests_error)
...
def make_item_from_policy_dict(data, page, tab_id):
    ...
    return {
        ...
        "policy_id": f"bokjiro_gyeonggi_{source_policy_id}",
...
def request_api_attempts(client: BokjiroClient, page: int, tab_id: str):
    ...
    for attempt in attempts:
        try:
            ...
            if page == 1:
                save_debug_response(
                    f"bokjiro_gyeonggi_tab{tab_id}_api_{attempt['name']}.txt",
                    text,
                )
            ...
            if page == 1:
                save_debug_response(
                    f"bokjiro_gyeonggi_tab{tab_id}_api_{attempt['name']}_error.txt",
                    str(e),
                )
...
def collect_policy_items_for_tab(client, tab_id, max_pages):
    ...
    for page in range(1, max_pages + 1):
        print(f"[복지로-경기] tabId={tab_id} API 목록 페이지 확인 중: {page}")
        ...
        for attempt_name, response_text in request_api_attempts(client, page, tab_id):
            ...
            if page_items:
                print(f"[복지로-경기] tabId={tab_id} 사용 API 방식: {attempt_name}")
                break

        print(f"[복지로-경기] tabId={tab_id} {page}페이지 정책 수:", len(page_items))

        if len(page_items) == 0:
            empty_policy_pages += 1
            print(f"[복지로-경기] tabId={tab_id} 연속 빈 목록 페이지:", empty_policy_pages)
        else:
            empty_policy_pages = 0

        if empty_policy_pages >= STOP_AFTER_EMPTY_POLICY_PAGES:
            print(f"[복지로-경기] tabId={tab_id} 빈 목록 페이지가 반복되어 목록 수집 종료")
            break

        results.extend(page_items)
        time.sleep(SLEEP_SECONDS)

    print(f"[복지로-경기] tabId={tab_id} 목록 전체 정책 수:", len(results))
    return results
...
def collect_policy_items(max_pages: int = MAX_PAGES):
    ...
    print("[복지로-경기] 목록 전체 정책 수:", len(all_items))
...
def fetch_policy_detail(client: BokjiroClient, item: dict):
    ...
    try:
        ...
        print("[복지로-경기] 상세 요청 실패, 목록 raw_text 사용:", item["title"], e)
...
def crawl_bokjiro_gyeonggi():
    ...
    for idx, item in enumerate(items, start=1):
        try:
            ...
            if row["crawl_status"] == "active":
                ...
                print(f"[복지로-경기] ACTIVE 저장 {len(active_rows)}개째:", row["title"], "/", row["crawl_reason"])

            elif row["crawl_status"] == "need_check":
                need_check_rows.append(row)
                print(f"[복지로-경기] NEED_CHECK 저장 {len(need_check_rows)}개째:", row["title"], "/", row["crawl_reason"])

            else:
                closed_rows.append(row)
                print(f"[복지로-경기] CLOSED 제외:", row["title"], "/", row["crawl_reason"])
            ...
            print("[복지로-경기] 상세 확인 실패, NEED_CHECK 처리:", item["title"], e)
    ...
    print("[복지로-경기] active raw 저장:", OUTPUT_ACTIVE_PATH)
    print("[복지로-경기] need_check raw 저장:", OUTPUT_NEED_CHECK_PATH)
    print("[복지로-경기] closed raw 저장:", OUTPUT_CLOSED_PATH)
    print("[복지로-경기] Ollama 입력용 raw 저장:", OUTPUT_OLLAMA_INPUT_PATH)

    print("[복지로-경기] active 수:", len(active_rows))
    print("[복지로-경기] need_check 수:", len(need_check_rows))
    print("[복지로-경기] closed 수:", len(closed_rows))
    print("[복지로-경기] Ollama 추출 대상 수:", len(final_rows))


if __name__ == "__main__":
    crawl_bokjiro_gyeonggi()