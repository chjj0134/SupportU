...
function normalizeDashboardApplyStatus(applyStatus: string) {
...
export function MyPage({ onNavigate }: MyPageProps) {
  ...
  const filteredCalendarEvents = calendarEvents.filter((event) =>
      isDashboardCategoryMatch(event.category, dashboardCategoryFilter),
  );

  const allAppliedCalendarEvents = calendarEvents.filter((event) =>
      normalizeDashboardApplyStatus(event.applyStatus) === "applied",
  );
  ...
  const benefitedCalendarEvents = filteredCalendarEvents.filter((event) =>
      normalizeDashboardApplyStatus(event.applyStatus) === "benefited",
  );
  ...
  const requiredActionCount = filteredCalendarEvents.filter((event) =>
      isOngoingCalendarEvent(event) && normalizeDashboardApplyStatus(event.applyStatus) === "apply_now",
  ).length;
  const monthlyAppliedCount = appliedCalendarEvents.filter((event) => isThisMonthCalendarEvent(event)).length;
  const waitingResultCount = filteredCalendarEvents.filter((event) =>
      isOngoingCalendarEvent(event) && normalizeDashboardApplyStatus(event.applyStatus) === "applied",
  ).length;
  const dashboardStatusCards = [
    ...
    { label: "결과 대기", sub: "심사 진행 중", value: `${waitingResultCount}건`, color: "#3b6661", bg: "rgba(59,102,97,0.1)" },
  ];
          ...
          <div className="flex items-center gap-4 mb-8 p-5 bg-white rounded-2xl border" style={{ borderColor: "rgba(226,232,240,0.8)", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
            ...
            <div className="ml-auto flex gap-6">
              {[
                ...
                { label: "지원 완료", value: allAppliedCalendarEvents.length.toString() },
