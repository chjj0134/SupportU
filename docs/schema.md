...
SupportU 프로젝트의 정책 데이터 수집 → 정규화 → DB 적재 과정에서 사용하는 공통 스키마를 정의한 문서입니다.
...
후가공 단계에서 공통 스키마(Common Schema)로 정규화한 뒤 Supabase에 적재합니다.
...
크롤러가 각 출처에서 수집한 원시 데이터를 아래 형태로 통일합니다.
각 출처의 필드명이 달라도 후가공 단계에서 아래 포맷으로 변환합니다.
...
후가공 단계에서 raw data를 파싱·LLM 처리하여 아래 공통 스키마로 변환합니다.
이 스키마가 Supabase `policies` 테이블의 입력값이 됩니다.
...
후가공 단계에서 아래 기준으로 중복 데이터를 제거합니다.
